         ...
         <div className="flex-1">
            {viewMode === 'grid' ? (
              <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-8">
                {MODS.map((mod) => (
                  <Link href={`/modlar/${mod.id}`} key={mod.id}>
                    <div className="group glass-panel rounded-[40px] border border-armoyu-card-border overflow-hidden hover:shadow-2xl transition-all duration-500 bg-armoyu-card-bg flex flex-col h-full">
              ...
              <div className="glass-panel overflow-hidden rounded-[32px] border border-armoyu-card-border shadow-2xl animate-in fade-in slide-in-from-right-8 duration-500">
                <div className="hidden md:grid grid-cols-12 gap-4 px-8 py-5 bg-black/5 dark:bg-white/5 border-b border-armoyu-card-border overflow-x-auto">
                ...
                <div className="divide-y divide-black/5 dark:divide-white/5">
                  {MODS.map((mod) => (
                    <Link href={`/modlar/${mod.id}`} key={mod.id}>
                      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 px-8 py-5 items-center hover:bg-blue-600/5 transition-all group">
                        <div className="col-span-1">
                        <img src={mod.image} className="w-12 h-12 rounded-xl object-cover border border-black/10 dark:border-white/10" alt="" />
                      </div>
                      <div className="col-span-4">
                        <h4 className="text-sm font-black text-armoyu-text uppercase tracking-tight group-hover:text-blue-500 transition-colors">{mod.name}</h4>
                      ...
                      <div className="col-span-1 text-right">
                        <button className="w-8 h-8 rounded-full bg-blue-600 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all ml-auto hover:scale-110">
