         ...
         <div className="w-full lg:w-72 shrink-0 space-y-8">
            <div className="glass-panel p-8 rounded-[40px] border border-armoyu-card-border bg-armoyu-card-bg">
               ...
               <div className="space-y-3">
                  {['Tüm Ürünler', 'Üyelik & VIP', 'Oyun İçi Paralar', 'Minecraft Eşyaları', 'Lisanslı Giyim', 'Dijital Kodlar'].map((cat, idx) => (
                    <button key={cat} className={`w-full text-left px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${idx === 0 ? 'bg-blue-600 text-white shadow-xl shadow-blue-500/20' : 'text-armoyu-text-muted hover:bg-black/5 dark:hover:bg-white/5 border border-transparent hover:border-armoyu-card-border'}`}>
                       {cat}
                    </button>
                  ))}
               </div>
